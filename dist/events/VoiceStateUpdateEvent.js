...
export let VoiceStateUpdateEvent = class VoiceStateUpdateEvent extends BaseEvent {
    async execute(oldState, newState) {
        if (this.client.config.debugMode) {
            ...
            const chDiff = oldState.channel?.id === newState.channel?.id ? [] : [
                [
                    "Channel",
                    `${oldCh} -> ${newCh}`
                ]
            ...
            const servMute = oldServM === newServM ? [] : [
                [
                    "Server Mute",
                    `${oldServM} -> ${newServM}`
                ]
            ...
            const selfMute = oldSelfM === newSelfM ? [] : [
                [
                    "Self Mute",
                    `${oldSelfM} -> ${newSelfM}`
                ]
            ...
            const servDeaf = oldServD === newServD ? [] : [
                [
                    "Server Deaf",
                    `${oldServD} -> ${newServD}`
                ]
            ...
            const selfDeaf = oldSelfD === newSelfD ? [] : [
                [
                    "Self Deaf",
                    `${oldSelfD} -> ${newSelfD}`
                ]
            ...
            this.client.debugLog.logData("info", "VOICE_STATE_UPDATE", [
                [
                    "Guild",
                    `${oldState.guild.name}(${oldState.guild.id})`
                ],
                [
                    "User",
                    oldState.member ? `${oldState.member.user.tag}(${oldState.member.user.id})` : "[???]"
                ],
                ...chDiff,
                ...servMute,
                ...selfMute,
        ...
        const queueVC = newState.guild.channels.cache.get(queue.connection.joinConfig.channelId);
        ...
        if (oldMember?.id === botID && oldID === queueVC.id && newID === undefined) {
            ...
            if (!isIdle) {
                ...
                queue.textChannel.send({
                    embeds: [
                        createEmbed("error", `⏹️ **|** ${i18n.__("events.voiceStateUpdate.disconnectFromVCMessage")}`)
                    ]
        ...
        if (member?.id === botID && oldID === queueVC.id && newID !== queueVC.id && newID !== undefined) {
            ...
            if (newVC?.type === "GUILD_STAGE_VOICE" && newState.suppress) {
                const msg1 = await queue.textChannel.send({
                    embeds: [
                        createEmbed("info", i18n.__("events.voiceStateUpdate.joiningAsSpeaker"))
                    ]
                });
                const suppress = await newState.setSuppressed(false).catch((err)=>({
                        error: err
                    }));
    ...
    timeout(vcMembers, queue, state) {
        ...
        state.guild.queue.timeout = setTimeout(()=>{
            ...
            void queue.textChannel.send({
                embeds: [
                    createEmbed("error", `⏹ **|** ${i18n.__mf("events.voiceStateUpdate.deleteQueue", {
                        duration: `\`${duration}\``
                    })}`).setAuthor({
                        name: i18n.__("events.voiceStateUpdate.deleteQueueFooter")
                    })
                ]
            });
        }, timeout);
        void queue.textChannel.send({
            embeds: [
                createEmbed("warn", `⏸ **|** ${i18n.__mf("events.voiceStateUpdate.pauseQueue", {
                    duration: `\`${duration}\``
                })}`).setAuthor({
                    name: i18n.__("events.voiceStateUpdate.pauseQueueFooter")
                })
            ]
        }).then((msg)=>queue.lastVSUpdateMsg = msg.id);
    }
    // eslint-disable-next-line class-methods-use-this
    resume(vcMembers, queue, state) {
        if (vcMembers.size <= 0) return;
        clearTimeout(queue.timeout);
        state.guild.queue.timeout = null;
        const song = queue.player.state.resource.metadata.song;
        void queue.textChannel.send({
            embeds: [
                createEmbed("info", `▶ **|** ${i18n.__mf("events.voiceStateUpdate.resumeQueue", {
                    song: `[${song.title}](${song.url})`
                })}`).setThumbnail(song.thumbnail).setAuthor({
...
VoiceStateUpdateEvent = __decorate([
