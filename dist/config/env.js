...
// Multiple values
export const altPrefixes = parseEnvValue(process.env.ALT_PREFIX || "{mention}").filter((x, i, a)=>a.indexOf(x) === i && x !== mainPrefix);
export const devs = parseEnvValue(process.env.DEVS ?? "");
...
export const presenceData = {
    activities: parseEnvValue(process.env.ACTIVITIES ?? "").map((x, i)=>({
            name: x,
            type: parseEnvValue(process.env.ACTIVITY_TYPES ?? "")[i]?.toUpperCase() || "PLAYING"
        })),
    status: [
        "online"
    ],
