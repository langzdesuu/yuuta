...
export let RemoveCommand = class RemoveCommand extends BaseCommand {
    async execute(ctx) {
        ...
        if (this.client.data.data?.[ctx.guild.id]?.dj?.enable && !ctx.member?.roles.cache.has(djRole?.id ?? "") && !ctx.member?.permissions.has("MANAGE_GUILD")) {
            void ctx.reply({
                embeds: [
        ...
        const pages = await Promise.all(chunk(songs, 10).map(async (v, i)=>{
            const texts = await Promise.all(v.map((song, index)=>`${isSkip ? i18n.__("commands.music.remove.songSkip") : ""}${i * 10 + (index + 1)}.) ${Util.escapeMarkdown(parseHTMLElements(song.song.title))}`));
            return texts.join("\n");
        }));
        const getText = (page)=>`\`\`\`\n${page}\`\`\``;
