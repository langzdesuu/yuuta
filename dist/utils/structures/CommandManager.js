...
export class CommandManager extends Collection {
    load() {
        fs.readdir(resolve(this.path)).then(async (categories)=>{
            this.client.logger.info(`Found ${categories.length} categories, registering...`);
            for (const category of categories){
                const meta = (await import(pathStringToURLString(resolve(this.path, category, "category.meta.js")))).default;
                this.categories.set(category, meta);
                this.client.logger.info(`Registering ${category} category...`);
                await fs.readdir(resolve(this.path, category)).then((files)=>files.filter((f)=>f !== "category.meta.js")).then(async (files)=>{
                    let disabledCount = 0;
                    ...
                    for (const file of files){
                        try {
                            ...
                            const command = await this.client.utils.import(path, this.client);
                            ...
                            if (!allCmd.has(command.meta.name) && command.meta.slash && this.client.config.enableSlashCommand) {
                                ...
                                await this.registerCmd(command.meta.slash, {
                            ...
                            this.client.logger.error(`Error occured while loading ${file}: ${err.message}`);
                        }
                    }
                    return {
                        disabledCount,
                ...
                }).catch((err)=>this.client.logger.error("CMD_LOADER_ERR:", err)).finally(()=>this.client.logger.info(`Done registering ${category} category.`));
            }
        }).catch((err)=>this.client.logger.error("CMD_LOADER_ERR:", err)).finally(()=>{
    ...
    handle(message, pref) {
        ...
        if (timestamps?.has(message.author.id)) {
            ...
            if (now < expirationTime) {
                ...
                message.channel.send({
                    embeds: [
                        createEmbed("warn", `⚠️ **|** ${i18n.__mf("utils.cooldownMessage", {
        ...
        try {
            ...
            if (command.meta.devOnly && !this.client.config.devs.includes(message.author.id)) return;
            this.client.logger.info(`${message.author.tag} [${message.author.id}] is using ${command.meta.name} command from ${command.meta.category} category ` + `on #${message.channel.name} [${message.channel.id}] in guild: ${message.guild.name} [${message.guild.id}]`);
        }
    }
    async registerCmd(data, options) {
        if (options && this.client.config.isDev) {
            for (const id of this.client.config.mainGuild){
                ...
                try {
                    ...
                    void options.onError(guild, err);
                }
            }
        } else {
