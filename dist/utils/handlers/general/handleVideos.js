...
export async function handleVideos(client, ctx, toQueue, voiceChannel) {
    const wasIdle = ctx.guild?.queue?.idle;
    async function sendPagination() {
    ...
    try {
        ...
        client.debugLog.logData("error", "HANDLE_VIDEOS", `Error occured while connecting to ${ctx.guild.name}(${ctx.guild.id}). Reason: ${error.message}`);
